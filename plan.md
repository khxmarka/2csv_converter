...
1. **Фильтр SQL.** В CSV попадают только `INSERT ... VALUES`, у которых имя таблицы совпало со словарём персональных данных (PII) либо среди колонок есть минимум две различные зачётные PII-колонки. Остальное тихо пропускать.
...
- или среди колонок есть не меньше двух различных зачётных PII-имён.

Иначе — пропустить тихо (как `INSERT ... SELECT` / `SET`). Соседние INSERT того же `.sql` это не спасают и не накапливаются.
...
| `INSERT INTO settings (email, status) …` | Не писать (одна PII-колонка) |
| `INSERT INTO settings (email, phone) …` | Писать (две PII-колонки) |
| `INSERT INTO t (user_id, email) …` | Не писать (технический ID не даёт зачёт) |
| `INSERT INTO t (national_id, email) …` | Писать |
...
- `Match(name string) bool`, `MatchColumns(names []string) bool`;
...
- колонки проходят только при ≥2 различных зачётных PII-именах; технические ID не считаются;
...
**Должны совпасть** (`Match` для имени таблицы; для колонок см. порог и технические ID):
...
`user_id` / `userid` совпадают как имя **таблицы**. Как колонка технический суффикс `id`/`uuid`/`guid` зачёта не даёт, если это не документный идентификатор.
